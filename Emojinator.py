...
class emojinator_model(object):
    
    def __init__(self):
        self.model = load_model('emojinator.h5')
        self.image_queue = queue.PriorityQueue()
        self.image_queue_idx = 0
    ...
    def emoji_open(self):
        if (self.emoji_status == 'None') or (self.emoji_status == 'Close'):
            print('Open the web cam')
            self.emoji_status = 'Open'
            self.emojis = self.get_emojis()
            self.cap = cv2.VideoCapture(0)
            self.x, self.y, self.w, self.h = 300, 50, 350, 350

    def emoji_close(self):
        if self.emoji_status == 'Open':
            self.emoji_status = 'Close'
            self.cap.release()

    def emoji_predict(self, is_run_model):
        pred_class = None
        ...
        if (self.cap.isOpened()):
            ...
            if ret:
                ...
                if is_run_model:
                    if len(contours) > 0:
                        ...
                        if cv2.contourArea(contour) > 2500:
                            ...
                            pred_probab, pred_class = self.keras_predict(self.model, newImage)
                ...
                # return the model prediction here
                return pred_class
    ...
    def overlay(self, image, emoji, x,y,w,h):
        emoji = cv2.resize(emoji, (w, h))
        try:
            image[y:y+h, x:x+w] = self.blend_transparent(image[y:y+h, x:x+w], emoji)
        except:
            pass
        return image
...
#     model_obj = emojinator_model()
#     model_obj.keras_predict(model_obj.model, np.zeros((50, 50, 1), dtype=np.uint8))
#     model_obj.run()
